...
public:
	...
	//递归渲染模型
	virtual void recursiveRender(const struct aiScene* sc, const struct aiNode* nd, float scale, bool isAmbient)
	{
...
private: 
	void recursiveRender(const struct aiScene* sc, const struct aiNode* nd, float scale, bool isAmbient) override
	{
		aiMatrix4x4 mTrans = nd->mTransformation;
		aiMatrix4x4 m2;
		aiMatrix4x4::Scaling(aiVector3D(scale, scale, scale), m2);
