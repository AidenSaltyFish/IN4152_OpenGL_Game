...
public:
	...
	//根据面包的位置判断是否能吃掉面包
	bool detectPlayerEatingApple(glm::vec3 applePos, float dist)
	{
		float dx = (applePos - cameraPos).x;
		float dy = (applePos - cameraPos).y;
		float dz = (applePos - cameraPos).z;
	...
	//判断玩家是否靠近某个面包
	bool detectPlayerCloseToApple(glm::vec3 applePos, float dist)
	{
		float dx = (applePos - cameraPos).x;
		float dy = (applePos - cameraPos).y;
		float dz = (applePos - cameraPos).z;
